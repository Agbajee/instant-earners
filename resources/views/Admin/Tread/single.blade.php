...
@section('content')
<form action="{{route('adminTreadsIDPostActions', [$tread->id, 2])}}" method="post" id="save_c">
        <form class="ecommerce-form action-buttons-fixed" method="post" action="{{route('adminCategoryEditIDPost', $tread->id)}}">
            @csrf
            <div class="row mt-2">
                <div class="col">
                                        ...
                                        <div class="tab-pane fade show active" id="category" role="tabpanel" aria-labelledby="category-tab">
                                            <div class="form-group row  pb-3">
                                                ...
                                                <div class="col-lg-12">
                                                        ...
                                                        <?php $the_other_cat = \App\Models\Category::all()->pluck('name', 'id'); ?>
    ...
    <div class="row my-4">
        ...
        <div class="d-flex flex-column card card-body">
            ...
            @if($tread->status == 1)
            <form action="{{route('adminTreadsIDPostActions', [$tread->id, 1])}}" id="saveDraft" method="post" class="btn btn-warning mb-2">
                @csrf
            ...
            @if($tread->status == 0)
            <form action="{{route('adminTreadsIDPostActions', [$tread->id, 4])}}" id="saveDraft" method="post"  class="btn btn-info mb-2">
            @csrf
                <button type="submit" id="saveDraftBtn" class="btn btn-info "> Make Tread Visible</button>
            </form>
            @endif
        
            <form action="{{route('adminTreadsIDPostActions', [$tread->id, 3])}}" method="post" class="btn btn-danger mb-2">
            @csrf
