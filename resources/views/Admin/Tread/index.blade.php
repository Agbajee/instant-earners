...
@section('content')
@php
    $dd1 = \App\Models\Treads::all();
    $dd = \App\Models\Treads::orderBy('created_at', 'DESC')->paginate(10);
    $pt = \App\Models\Treads::where('status', 1)->get();
    $dt = \App\Models\Treads::where('status', 0)->get();
    $counter = 1;
@endphp
								...
								<div class="col-12 col-lg-auto mb-3 mb-lg-0">
									<div class="d-flex align-items-lg-center flex-column flex-lg-row">
										...
										<select class="form-control select-style-1 filter-by" name="filter-by">
											...
											<option><a href="{{route('adminTreadsPublished')}}">Published ({{$pt->count()}})</a></option>
											<option><a href="{{route('adminTreadsDraft')}}">Drafts ({{$dt->count()}})</a></option>
										</select>
									</div>
								</div>
						...
						<table class="table table-ecommerce-simple table-striped mb-0" id="datatable-ecommerce-list" style="min-width: 550px;">
							...
							<tbody>
								...
								<tr>
									...
									<td class="actions">
										<a href="{{route('adminTreadsSelected', $ui->id)}}" class="btn btn-sm btn-danger text-light"><i class="bx bx-trash"></i> </a>
										<a href="{{route('adminTreadsID', $ui->id)}}" class="btn btn-sm btn-info text-light"><i class="bx bx-pen"></i> </a>
									</td>
								</tr>
								@endforeach
							</tbody>
